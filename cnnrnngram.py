...
class cnnattmodel(object):
    def __init__(self,config,is_training,embedding):       
        ...
        #改为max-pooling
        maxwrecx=tf.nn.max_pool(wrecx,ksize=(1,self.wordsnum-self.ngram+1,1,1),strides=[1,1,1,1],padding='VALID')#[batch_size, 1, 1, num_filters]
        ...
        ssdocp=tf.reduce_max(cnnrhidden_sen_1,1) #batch hidden
        '''
...
#---------------------------------------------------------------------------------        
        ...
        rsenp=tf.reduce_max(rhidden_state_1,1)#batch*sens,hid
        '''
        #attention
        rhidden_state_1=tf.stack(self.rwoutput,axis=1) #batch*sens,words,wordtosen_hidsize
        ...
        #sentence level
        rinputss=tf.reshape(rsenp,[-1,self.sentencesnum,self.wordtosen_hidsize])#bathc,sens,hid
        ...
        rdocp=tf.reduce_max(rhidden_sen_1,1)
        '''
        #attention
        rhidden_sen_1=tf.stack(rsoutput,axis=1) #batch,sens,hidden
...
#------------------------------------------------------------     
    ...
    def initial_weight(self):        
         ...
         with tf.name_scope("final"):
             self.w1=tf.get_variable("w22",shape=[self.sentotext_hidsize*2,self.classnum])
